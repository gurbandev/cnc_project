                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('category_id') is-invalid @enderror" name="category_id" id="category_id" required autofocus>
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{$category->id ==  $obj->category_id ?'selected':''}}>{{ \App\Http\Controllers\CategoryController::getCategoryTree($category, $category->name) }}</option>
                        @endforeach
                    </select>
                    @error('category_id')
                        ...
                        <select class="form-select @error('working_area_bit_id') is-invalid @enderror" name="working_area_bit_id"
                            ...
                            @foreach($attributes as $attribute)
                                @if($attribute->attribute_id == 3)
                                    <option value="{{ $attribute->id }}" {{$obj->working_area_bit_id  == $attribute->id}}>{{ $attribute->name }}</option>
                                @endif
                            @endforeach
                        </select>
